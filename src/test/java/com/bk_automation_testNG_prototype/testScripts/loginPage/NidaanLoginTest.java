...
public class NidaanLoginTest extends TestBase{
	...
	public void verifyLogin_ValidCredentials(String userType,String username, String password) {
		...
		//test = extent.createTest(method.getName()+"_"+userType);
		
		switch(userType) {
		
		case "Patient" : logExtentReport("Verifying login functionality for Patient");
		...
		case "Doctor" :  logExtentReport("Verifying login functionality for Doctor");
						 login.loginIntoApplication(username, password);
		...
		case "Hospital" : logExtentReport("Verifying login functionality for Hospital");
						  login.loginIntoApplication(username, password);
	...
	//@Test(dataProvider="InvalidtestData", priority = 2, description = "Login with valid credentials")
